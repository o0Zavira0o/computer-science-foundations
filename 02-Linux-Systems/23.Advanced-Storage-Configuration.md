...
# Filesystem Management: Configuration, Capacity, and Usage Analysis

Now we will cover modern configuration alternatives, the **`df`** (Disk Free) command, the **`du`** (Disk Usage) command, and the mathematics behind hidden "reserved" storage.

---

## Part 1: Alternatives to `/etc/fstab`
While `/etc/fstab` is the classic standard, modern Linux systems provide two other ways to manage mount points.

### 1.1 The `/etc/fstab.d/` Directory
Instead of one giant file with 50 lines, some systems allow you to split configurations into small, individual files inside a directory named `/etc/fstab.d/`.
*   **Benefit:** Easier automation (tools can add/remove a single file without parsing the main list).
*   **Concept:** This follows the Linux pattern of "Drop-in directories" (like `/etc/apt/sources.list.d/`).

### 1.2 Systemd Units
Most modern Linux distributions use **systemd** as the init system (the "boss" process that starts everything). Systemd uses "Unit files" to control services.
*   **Mount Units:** You can create a file ending in `.mount` (e.g., `home-user-data.mount`).
*   **Relationship:** often, when you write lines in `/etc/fstab`, the system automatically converts them into systemd units in the background during boot.

---

## Part 2: Checking Capacity (`df`)
To see the "Gas Gauge" of your storage devices, we use the `df` command.

### 2.1 The Fields
When you run `df`, you get columns:
1.  **Filesystem:** The device name (e.g., `/dev/sda1`).
2.  **1k-blocks:** Total size (measured in 1024-byte chunks).
3.  **Used:** Space occupied.
4.  **Available:** Space free for use.
5.  **Use%:** Percentage full.
6.  **Mounted on:** The directory path.

### 2.2 The "Missing" Space (Reserved Blocks)
You might notice that the math doesn't add up.
$$ \text{Total} \neq \text{Used} + \text{Available} $$

**Why?**
Linux filesystems (specifically ext4) secretly reserve **5%** of the disk space for the **root user** only.
*   **Purpose:** If a user fills the hard drive to 100%, system services (logs, daemons) would crash. The 5% buffer ensures the OS has room to function while you clean up the mess.

---

## Part 3: Checking Usage (`du`)
While `df` looks at the *whole disk*, `du` looks at *specific directories*. It calculates the weight of files in a folder.

*   **Recursive:** By default, it crawls into every sub-folder.
*   **Summary (`-s`):** Instead of listing every single file, it just gives you the total size of the folder.

---

## Part 4: Units and Readability
Computers count in bytes. Humans count in Megabytes (MB) or Gigabytes (GB).

### 4.1 The Flags
*   **`-k`**: Display in Kilobytes (1024 bytes). This is usually the default.
*   **`-m`**: Display in Megabytes.
*   **`-h` (Human Readable):** The most useful flag. It automatically converts numbers to KB, MB, or GB depending on size.

### 4.2 POSIX Standards
Technically, the "Standard" block size is **512 bytes**. However, 1024 bytes (1KB) is easier to read, so Linux defaults to that.
If you set the environment variable `POSIXLY_CORRECT`, Linux will force tools to display 512-byte blocks.

---

## Part 5: Hands-on Lab (GitHub Codespaces)

In this lab, we will create a simulated disk, fill it with data, and analyze the space using `df` and `du` to see the "Reserved Space" in action.

### Step 1: Create a Virtual Disk
We will create a small 100MB file to act as a hard drive.
```bash
# Create a 100MB file
dd if=/dev/zero of=storage_test.img bs=1M count=100

# Format it as ext4
mkfs.ext4 -F storage_test.img
```
...
# Mount the disk image to the folder
sudo mount storage_test.img my_storage
```

### Step 3: Analyze Capacity (`df`)
Now, let's look at the numbers.

```bash
# Check disk free space for this specific folder
df my_storage
```
**Observation:**
Look at the **Available** column.
*   Total Size: $\approx 97,000$ (1K-blocks) $\approx 100 \text{MB}$.
*   Used: Very small (metadata).
*   Available: It will be significantly less than Total - Used.
*   *Why?* Because $\approx 5\%$ is reserved for root.
...
df -h my_storage
```
*   Now you should see units like "M" (Megabytes).

### Step 5: Fill the Disk (`du`)
We will create a 10MB file inside the disk and weigh it.

```bash
# Create a 10MB file inside the mounted drive
sudo dd if=/dev/zero of=my_storage/big_file.dat bs=1M count=10

# Check usage of the directory
# -h = Human readable
# -s = Summary (don't list every file, just the total)
du -sh my_storage
```
*Result:* It should report roughly 10MB used.

### Step 6: Targeted Analysis
Often, you don't know *where* the space is going. The combination of `du` and wildcards `*` helps.

```bash
# Create another file
sudo dd if=/dev/zero of=my_storage/another_file.dat bs=1M count=5

# Check usage of everything inside the folder
cd my_storage
du -h *
```
*Result:* This lists each file individually so you can find the largest one.

### Step 7: The POSIX Variable
Let's see how environment variables change the output numbers.

```bash
# Default Linux view (1024 byte blocks)
df .

# Strict POSIX view (512 byte blocks)
POSIXLY_CORRECT=1 df .
```
**Observation:** The numbers in the second command are exactly double the numbers in the first command.
