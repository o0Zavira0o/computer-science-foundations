<!-- GENERATED by scripts/csf.py sync. Fast handoff view; canonical JSON remains authoritative. -->

# Neurotechnology & Neural Engineering — AI Context

## Identity

- **Track:** Neurotechnology & Neural Engineering
- **ID:** `neurotechnology-neural-engineering`
- **Code:** `NNE`
- **Track status:** active
- **Entry model:** zero-subject-specific-knowledge
- **Focus:** Neurotechnology and neural engineering from first principles: how nervous-system signals arise, how they are measured and modulated, how neural interfaces and neuroprosthetic systems are engineered, and how biological, computational, device, clinical, regulatory, human-factors, and neuroethical constraints interact.

## Curriculum state

- **Audit status:** audited
- **Nodes:** 89
- **Coverage baseline:** audited
- **Coverage items:** 10
- **Published lessons:** 20

## Registry sizes

- **Concepts:** 92
- **Examples:** 98
- **References:** 105

## Learner state

- **Tracked lessons:** 0
- **Review queue:** 0
- **Last updated:** never

## Authoring candidates

- `NNE-N-0021` — fMRI and the BOLD signal: hemodynamic inference rather than direct voltage recording (L1, planned)

## Learner next actions

**New lessons with practiced/demonstrated prerequisites**
- NNE-0001 — What neurotechnology and neural engineering are actually studying [L0]

## Recent lessons

- `NNE-0013` — The electrode-tissue interface: charge transfer, impedance, and what an electrode actually senses (L1, complete)
- `NNE-0014` — Intracellular recording and patch clamp as a window into membrane voltage and currents (L1, complete)
- `NNE-0015` — Extracellular spikes, multi-unit activity, and local field potentials (L1, complete)
- `NNE-0016` — Microelectrode arrays and high-channel-count invasive recording (L1, complete)
- `NNE-0017` — ECoG and intracranial EEG: recording from the cortical surface and depth (L1, complete)
- `NNE-0018` — Scalp EEG: potentials, montages, volume conduction, and spatial ambiguity (L1, complete)
- `NNE-0019` — Peripheral neural and neuromuscular signals: ENG and EMG (L1, complete)
- `NNE-0020` — MEG: magnetic fields generated by neural currents (L1, complete)

## Normal-session retrieval rule

Do not recursively reread the whole track. Inspect the canonical JSON above, then open only the direct prerequisite/recent/cross-track lessons and any lessons found by relevant concept/example/reference IDs.

For full coverage or semantic-duplication audits, explicitly switch to audit mode.
